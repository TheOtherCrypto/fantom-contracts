...
contract ProtofiMasterChef is Ownable, ReentrancyGuard {
    ...
    bool private _isPoolAddressSet = false;
    ...
    event UpdatedPoolAddress(address indexed newPoolAddress);
    ...
    function updatePool(uint256 _pid) public {
        ...
        // 10% fees to the dev address
        // Of this 10%, 5% is for salaries + Marketing + Moneypot
        proton.mint(devAddress, protonReward.div(10));

        if(_isPoolAddressSet && nftPoolFeeBP > 0){
            // This additional fee goes for the NFT APR boost
            proton.mint(poolAddress, protonReward.mul(nftPoolFeeBP).div(10000));
        }
    ...
    */
    ...
    function setPoolAddress(address _poolAddress) public onlyOwner{
        ...
        _isPoolAddressSet = true;
        emit UpdatedPoolAddress(_poolAddress);
    }

    // Sets Proto to mint for the NFT pool, can be changed only by the owner.
